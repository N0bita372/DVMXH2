...
function base_url($url = '')
{
    ...
    if ($a == 'http://localhost') {
        $a = 'http://localhost/CMSNT.CO/DVMXH2';
    }
    return $a.'/'.$url;
}
...
function active_category($slug){
    $CMSNT = new DB;
    foreach ($slug as $row) {
        if (isset($_GET['slug']) && $_GET['slug'] == $row) {
            return 'active';
        }
        $category = $CMSNT->get_row("SELECT * FROM `categories` WHERE `slug` = '$row' ");
        if($CMSNT->get_row(" SELECT * FROM `posts` WHERE `category_id` = '".$category['id']."' AND `display` = 1 AND `slug` = '".check_string($_GET['slug'])."' ")){
            return 'active';
        }
    }
}
